...
with tabs[0]:
    ...
    if st.button("Generate vCard & QR"):
        ...
        with zipfile.ZipFile(zip_buf, "w") as zf:
            ...
            zf.writestr(f"{fname}/{fname}_qr.png", buf.getvalue())
...
with tabs[2]:
    ...
    encryption = st.selectbox("Encryption", ["WPA", "WEP", "nopass"], index=0)
    if st.button("Generate WiFi QR"):
        ...
        st.download_button("⬇️ WiFi QR PNG", data=buf.getvalue(), file_name=f"{fname}.png", mime="image/png")

    # --- Event ---
    st.subheader("📅 Event QR")
    ev_title = st.text_input("Event Title")
    ev_start = st.text_input("Start (YYYYMMDDTHHMMSSZ)", value="20250925T130000Z")
    ev_end = st.text_input("End (YYYYMMDDTHHMMSSZ)", value="20250925T140000Z")
...
END:VEVENT"""
        ...
        st.download_button("⬇️ Event QR PNG", data=buf.getvalue(), file_name=f"{fname}.png", mime="image/png")
    ...
    if st.button("Generate MeCard QR"):
        ...
        st.download_button("⬇️ MeCard QR PNG", data=buf.getvalue(), file_name=f"{fname}.png", mime="image/png")

    # --- Crypto ---
    st.subheader("💰 Crypto Payment QR")
    coin = st.selectbox("Cryptocurrency", ["bitcoin", "ethereum"], index=0)
    ...
    if st.button("Generate Crypto QR"):
        ...
        st.download_button("⬇️ Crypto QR PNG", data=buf.getvalue(), file_name=f"{fname}.png", mime="image/png")
...
Developed by Abdulrrahman Alowain | 
<a href="https://x.com/a_owain" target="_blank">Follow Me</a>
</p>
""", unsafe_allow_html=True)