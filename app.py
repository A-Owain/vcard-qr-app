...
with tabs[0]:
    ...
    if st.button("Generate QR", key="tab1_generate"):
        if qr_text.strip():
            ...
            st.image(io.BytesIO(png), caption="QR Code", use_column_width=False)
            ...
            st.warning("⚠️ Please enter text or URL", icon="⚠️")
...
with tabs[1]:
    ...
    mapslink = st.text_input("MapsLink", key="tab2_mapslink")
    ...
    if st.button("Generate vCard", key="tab2_generate"):
        ...
        st.download_button("📥 Download vCard (.vcf)", data=vcard, file_name=f"{first}_{last}.vcf",
                           mime="text/vcard", key="tab2_dl_vcf")
...
with tabs[2]:
    ...
    custom_suffix_qr = st.text_input("Optional: Custom suffix", key="tab3_suffix")
    if st.button("🚀 Generate Batch QR Codes", key="tab3_generate"):
        if uploaded_qr_excel:
            ...
            st.text_area("📋 Summary", summary, height=200, key="tab3_summary")
            st.download_button("📥 Download ZIP", data=zip_buf, file_name=f"{os.path.basename(batch_folder)}.zip",
                               mime="application/zip", key="tab3_dl_zip")
...
with tabs[3]:
    ...
    custom_suffix_vc = st.text_input("Optional: Custom suffix", key="tab4_suffix")
    if st.button("🚀 Generate Batch vCards", key="tab4_generate"):
        if uploaded_emp_excel:
            ...
            zip_buf = zip_directory(batch_folder)
            st.text_area("📋 Summary", summary, height=200, key="tab4_summary")
            st.download_button("📥 Download ZIP", data=zip_buf, file_name=f"{os.path.basename(batch_folder)}.zip",
                               mime="application/zip", key="tab4_dl_zip")
...
with tabs[4]:
    ...
    if st.button("Generate Barcode", key="tab5_generate"):
        if bc_data.strip():
            ...
            st.image(io.BytesIO(png), caption="Barcode", use_column_width=False)
            st.download_button("📥 Download Barcode", data=png, file_name=f"{bc_type}_barcode.png",
                               mime="image/png", key="tab5_dl")
        else:
            st.warning("⚠️ Please enter data", icon="⚠️")
...
with tabs[5]:
    st.download_button("📥 Download Employee Directory Template", data=generate_employee_template_xlsx(),
                       ...
                       mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", key="tab6_dl")
...
with tabs[6]:
    st.download_button("📥 Download Batch QR Template", data=generate_batch_qr_template_xlsx(),
                       ...
                       mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", key="tab7_dl")
